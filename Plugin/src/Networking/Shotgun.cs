...
public static class Shotgun
{
    ...
    {
        NetworkManager.Singleton.CustomMessagingManager.RegisterNamedMessageHandler(SyncAmmoServerRpcMessage, OnSyncAmmoServerRpc);
        NetworkManager.Singleton.CustomMessagingManager.RegisterNamedMessageHandler(SyncAmmoClientRpcMessage, OnSyncAmmoClientRpc);
        NetworkManager.Singleton.CustomMessagingManager.RegisterNamedMessageHandler(SyncSafetyServerRpcMessage, OnSyncSafetyServerRpc);
        NetworkManager.Singleton.CustomMessagingManager.RegisterNamedMessageHandler(SyncSafetyClientRpcMessage, OnSyncSafetyClientRpc);
    ...
    public static void SyncAmmoServerRpc(NetworkObjectReference shotgunReference, int ammoCount)
    {
        ...
        NetworkManager.Singleton.CustomMessagingManager.SendNamedMessage(SyncAmmoServerRpcMessage, NetworkManager.ServerClientId, buffer);
    }
    ...
    private static void OnSyncAmmoServerRpc(ulong senderId, FastBufferReader data)
    {
        ...
        AdditionalNetworking.VerboseLog(LogLevel.Debug, () => $"{nameof(Shotgun)}.SyncAmmoServerRpc was called for {shotgunReference.NetworkObjectId}! ammo: {ammoCount}");
        
        SyncAmmoClientRpc(shotgunReference, ammoCount);
    }


    private static void SyncAmmoClientRpc(NetworkObjectReference shotgunReference, int ammoCount, ulong[] targets = default)
    {
        var buffer = new FastBufferWriter(1024, Allocator.Temp);
        buffer.WriteNetworkSerializable(shotgunReference);
        buffer.WriteValue(ammoCount);
    ...
    private static void OnSyncAmmoClientRpc(ulong senderId, FastBufferReader data)
    {
        ...
        AdditionalNetworking.VerboseLog(LogLevel.Debug, () => $"{nameof(Shotgun)}.SyncAmmoClientRpc was called for {shotgunReference.NetworkObjectId}! ammo: {ammoCount} was: {shotgunItem.shellsLoaded}");
    ...
    public static void SyncSafetyServerRpc(NetworkObjectReference shotgunReference, bool safety)
    {
        ...
        NetworkManager.Singleton.CustomMessagingManager.SendNamedMessage(SyncSafetyServerRpcMessage, NetworkManager.ServerClientId, buffer);
    }
    ...
    private static void OnSyncSafetyServerRpc(ulong senderId, FastBufferReader data)
    {
        ...
        if (!shotgunReference.TryGet(out var networkObject) || (senderId != NetworkManager.ServerClientId && networkObject.OwnerClientId != senderId))
            return;
        
        AdditionalNetworking.VerboseLog(LogLevel.Debug, () =>  $"{nameof(Shotgun)}.SyncSafetyServerRpc was called for {shotgunReference.NetworkObjectId}! safety:{(safety ? "on" : "off")}");

        SyncSafetyClientRpc(shotgunReference, safety);
    }


    private static void SyncSafetyClientRpc(NetworkObjectReference shotgunReference, bool safety, ulong[] targets = default)
    {
        var buffer = new FastBufferWriter(1024, Allocator.Temp);
        buffer.WriteNetworkSerializable(shotgunReference);
        buffer.WriteValue(safety);
        if (targets == default)
            ...
            NetworkManager.Singleton.CustomMessagingManager.SendNamedMessage(SyncSafetyClientRpcMessage, targets, buffer);
    }
    ...
    private static void OnSyncSafetyClientRpc(ulong senderId, FastBufferReader data)
    {
        ...
        AdditionalNetworking.VerboseLog(LogLevel.Debug, () =>  $"{nameof(Shotgun)}.SyncSafetyClientRpc was called for {shotgunReference.NetworkObjectId}! safety:{(safety ? "on" : "off")} was: {(shotgunItem.safetyOn ? "on" : "off")}");
    ...
    private static void OnRequestSyncServerRpc(ulong senderId, FastBufferReader data)
    {
        ...
        AdditionalNetworking.VerboseLog(LogLevel.Debug, () => $"{nameof(Shotgun)}.RequestSyncServerRpc was called for {shotgunReference.NetworkObjectId} by {senderId}!");
